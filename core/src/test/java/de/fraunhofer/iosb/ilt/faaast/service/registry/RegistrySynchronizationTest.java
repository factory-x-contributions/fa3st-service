...
public class RegistrySynchronizationTest {
    ...
    // Introduce this to ensure any given configuration URL path is not removed
    ...
    public void init() throws Exception {
        ...
        registrySynchronization = new RegistrySynchronization(
                CoreConfig.builder()
                        ...
                        .submodelRegistry("http://localhost:" + wireMockRule.port() + API_PREFIX)
                        .build(),
                persistence,
                messageBus,
                List.of(endpoint));
    ...
    public void testSubmodelCreation() throws MessageBusException, JsonProcessingException, SerializationException {
        ...
        registrySynchronization.stop();
        verify(postRequestedFor(urlEqualTo(API_PREFIX + SUBMODEL_URL_PATH))
    ...
    public void testSubmodelUpdate() throws MessageBusException, JsonProcessingException, SerializationException {
        ...
        messageBus.publish(ElementUpdateEventMessage.builder()
                .element(submodel).build());
        registrySynchronization.stop();
        verify(putRequestedFor(urlEqualTo(API_PREFIX + SUBMODEL_URL_PATH + "/" + EncodingHelper.base64UrlEncode(submodel.getId())))
                .withRequestBody(equalToJson(getSubmodelDescriptorBody(submodel), true, false)));
    ...
    public void testSubmodelDeletion() throws MessageBusException {
        ...
        messageBus.publish(ElementDeleteEventMessage.builder()
                .element(submodel).build());
        registrySynchronization.stop();
        verify(deleteRequestedFor(urlEqualTo(API_PREFIX + SUBMODEL_URL_PATH + "/" + EncodingHelper.base64UrlEncode(submodel.getId()))));
    }
