...
| tokenExchange<br>*(optional)*           | String                                                      | The base URL of the Security Token Service (STS) for token exchange. When set, incoming JWT tokens are exchanged for a new access token by attaching /token and verified by attaching /jwks to this base Url. Overrides `jwkProvider`.|                                             |
