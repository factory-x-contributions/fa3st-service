...
| callbackAddress<br>*(optional)*         | String                                                      | The callback URI to be used for automatic registration with registry. **Overrides `hostname`.** This variable replaces all parts of the automatically registered URI **except** the trailing path (e.g., /submodels/{id})                   | auto-detect (typically IP address)          |
